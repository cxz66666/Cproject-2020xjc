...
void polyfit2(int n, double* x, double* y, int poly_n, double a[]);//polyfit2为以后的拓展方向
...
void gauss_solve(int n, double A[], double x[], double b[])   //解高斯方程
{
    int i, j, k, r;
    double max;
    for (k = 0; k < n - 1; k++)
...
int CalY(int num, double  c[]) {
    double ans = 0;
    if (PolyNum) {
        for (int i = 0; i <= PolyNum; i++) {    //如果是多项式拟合  就按这样算
        ans += pow(num, i) * c[i];
        }
    }
