...
static double PolyPredictData[MAXCOLUMN + 1][10]; //存放每个数据的拟合阶数     第一个括号为列数  第二个括号为每项的系数  其中 第0项放的是拟合系数  之后 1，2，3...依次防止常数项，一次项，以此类推

static BOOL CheckPredict(string WrongTip, string InputMonth, string InputDay, string DateLength, string PredictLength, string ploy, int ans[4]);
static BOOL FindDate(int month, int day);

static void polyfit1(int n, double* x, double* y, int poly_n, double a[]); //a中放的是返回的系数  3阶以上就暴毙
static void gauss_solve(int n, double A[], double x[], double b[]);        //配合polyfit1食用

static void polyfit2(int n, double* x, double* y, int poly_n, double a[]); //这里先把2写出来，有机会的话再进行添加

static void PreparePredict(CaseNode_Ptr HEAD, int Date[4]);

static void FindBestPredict(int n, double* x, double* y, double a[]);
...
static void polyfit1(int n, double x[], double y[], int poly_n, double a[])
{
    int i, j;
...
{ //预测数据的链表也带头节点
    ...
    while (tmp1 != NULL && HaveNum < Date[2])
    { //date【2】里是应该采集的数据量
        HaveNum++;
        tmp1 = tmp1->next;
    ...
    for (i = 1; i <= TotalColumnNum; i++)
    {
        ...
        while (tmp1 != NULL)
        {
            ...
            if (nownum <= Date[2])
                ...
                strcpy(tmp1->Changedcolor, "Dark Gray");
            tmp1 = tmp1->next;
        }
    ...
    if (IsPredict)
    {
        ...
        for (i = 0; i <= PolyPredictData[NowChooseColumn][0]; i++)
        {
            if (!i) //常数项
                ...
                sprintf(OutEquationTmp, "%.1lfx^%d", PolyPredictData[NowChooseColumn][i + 1], i); //二次以上的项
...
static void DrawOldPoint(double PointData[][2], int Num)
{
    int i;
    SetPenColor("Blue");
