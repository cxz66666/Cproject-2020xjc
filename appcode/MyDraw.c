...
void DrawLeftButton(CaseNode_Ptr Head); //左侧的选择日期 选择列的显示
void drawMainPicture();
void DrawEachDate(int tmpnum, char *Date, CaseNode_Ptr ptr);      //画每一天的日期
void DrawDate(CaseNode_Ptr HEAD);                                 //画下方的日期
...
void DrawPicture(CaseNode_Ptr Head)
{
    ...
    if (IsRedisplay)   //如果需要重新刷新的话 直接清屏
    {
        IsRedisplay = 0;
        display();
    }
...
void DrawStatus()
{
    ...
    SetPenColor("Black");
    char NowStatus[100];
    memset(NowStatus, 0, sizeof(NowStatus));   //初始化        
    SetPointSize(15);
    /*以下注释几乎就是汉字*/
    if (IsSave)                          //对每个状态进行判断
...
void DrawOpenDir()
{
    ...
    if (button(GenUIID(0), MaxX*0.83,MaxY*0.9-2*BoxHeight, 2,FontHeight * 2, "打开"))   //使用现成的工具 win给的api直接打开
    {
        ...
        if (FileDialog(szFile))   //winapi
        {
            if (CheckFileName(szFile))   //检测是否是csv文件
            {
                ...
                {
                    ResetFileName(str);  //切换现在的filename
                    NowShowTable = FileHead;   //切换
                    IsOpen = 2;  //可以进行打开
    ...
    else
    {
        ...
        for (i = 1; i <= FileNum; i++)
        {
            ...
            if (button(GenUIID(i - 1), MaxX / 2 - TextStringWidth(NameDemo), BeginY - i * BoxHeight * 1.5 + 0.25 * BoxHeight, TextStringWidth(NameDemo) * 2, BoxHeight, FileName[i - 1]))
            {
                ...
                //printf("ok\n");
                if (ReadCSVFile(FileName[i - 1])) //读取文件啊
                {
                    if (!FileTotalNum) //如果当下没有读取到
                    {
    ...
    {
        memset(ErrorAns, 0, sizeof(ErrorAns));
        MyError = 0;   //error的报错提示
        if (CheckName(dirName))   //首先检查文件名字
        { 
            ResetFileName(dirName);  //检查成功后重置

            if (ReadCSVFile(dirName))   //进行文件读取
            {
                if (!FileTotalNum)   //没读到文件
                { 
                    SetPenColor("Red");
                ...
                else
                {
                    NowShowTable = FileHead;
                    IsOpen = 2;   //打开文件
                    ChangeIsSelect(NowShowTable);  //改变日期最多选20个
                    Calculate(NowShowTable);
                    return;
                }
            }
...
{
    CaseNode_Ptr tmp = FileTail; //注意FileTail放的是所有选中日期的最后一个节点
    int i;
    for (i = 1; i <= ChooseColumnNum; i++)
    {
        ...
        { //如果该列是选中的直线或者柱状图
            ...
            CaseNode_Ptr tmp1 = NowShowTable->next; //开始将每个选中的日期都画数字
