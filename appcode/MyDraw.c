...
*/
void display(); //����Ҫ��չʾ����
void DrawPicture(stu_Ptr Head); //������Ҫ��ͼ
...
void DrawLeftButton(stu_Ptr Head);//����ѡ������ ѡ���е���ʾ
void drawMainPicture();
void DrawEachDate(int tmpnum, char* Date,stu_Ptr ptr);//��ÿһ�������
...
void display()
{
    if (!IsSave&&!IsNew)   //������ڱ��������  ��update  ��Ϊ������������������  ʹ��update�޷���������
        UpdateDisplay();
    DisplayClear();//����
...
{   
   DisplayClear();
    if (IsHelp == 1){
     ...
     else if (IsNew == 1) {       //չʾͼ���м������   ��������½�  �����½������ Isopendirѡ��򿪵�
...
{   
    ...
    if (button(GenUIID(0), 0.9 * MaxX, 0.8 * MaxY, 1, FontHeight * 2, "��")) {
            char szFile[MAX_PATH] = { 0 };
            memset(ErrorAns, 0, sizeof(ErrorAns));  //����ÿ��������ʱ�����ErrorAns
            ...
            if (button(GenUIID(i - 1), MaxX / 2 - TextStringWidth(NameDemo), BeginY - i * BoxHeight * 1.5 + 0.25 * BoxHeight, TextStringWidth(NameDemo) * 2, BoxHeight, FileName[i - 1]))
            {
                ...
                if (ReadCSVFile(FileName[i - 1]))  //read��ȷ����1   ����ȷ����0
                {
                    ...
                    else
                    {
                        ...
                        ChangeIsSelect(NowShowTable);  //ѡ���20������
                        Calculate(NowShowTable);
    ...
    if (button(GenUIID(0), MaxX / 2 + 1.5, BeginY - FileNum * BoxHeight * 1.5 - 2 * BoxHeight, 1, BoxHeight, "ȷ��"))
    {
        ...
        {
            SetPenColor("Red");
            strcpy(ErrorAns, "��ȷ����������ȷ���ļ���Ŷ");
           
        }
    }
    SetPenColor("Red");   //������������ֱ�����   ���оͽ���handle��ı�
    if (strlen(ErrorAns))
    {
        printf("%s\n", ErrorAns);
...
void DrawLeftButton(stu_Ptr Head)
{
    ...
    {   
        ...
        setButtonColors("Black", "Black", "Red", "Red", IsSelect);//ע���Ƿ���Ϳ
        nowNum++;
       /*�����趨10������ʾ��ҳ   ֻ��ʾ��ǰʮ��*/
    ...
    for (i=NowDateColumn*4+1; i <= TotalColumnNum&&i<=(NowDateColumn+1)*4; i++)
    {
        ...
        if ((NowDateColumn + 1) * 4 < TotalColumnNum) { //����������һҳ
            setButtonColors("DirSelectionFrame", "Black", "DirSelectionFrameHot", "Black", 0);
    ...
    if (IsChangeNum&&ChangingPtr!=NULL) {
        ...
        {
            if (CheckChangedNum(ChangingPtrStringNum, ErrorAns)) {//�����ݺͷ��ص���ʾ������
...
*/
void DrawLastTableNum() {
    stu_Ptr tmp = tail;//ע��tail�ŵ�������ѡ�����ڵ����һ���ڵ�
    int i;
    for (i=1; i <= ChooseColumnNum; i++) {
        int column = ChoosedColumn[i];
            ...
            else  if (column == ChooseLineNum||column==ChooseHistogramNum) { //���������ѡ�е�ֱ�߻�����״ͼ
               
               
                stu_Ptr tmp1 = NowShowTable->next;//��ʼ��ÿ��ѡ�е����ڶ�������
...
{  
    ...
    for (i = 1; i <=ChooseColumnNum; i++) {
        int column = ChoosedColumn[i];
           ...
           IsChooseColumn[column]==1? SetPenSize(2):SetPenSize(5);     //���������߻�����   1��������  2����
...
void HandleHelpButton(int selection){ //�˵���HELP���Ŀ��ƺ��� 
	if (selection == 1){
		IsHelp=1;
		IsNew=0;
		IsOpen=0;
...
void DrawHelpPage1(){  //����ҳ���һҳ 
	    ...
	    drawLabel(MaxX-interval*68,MaxY-interval*14,"�ڴ��ļ�ǰ�����Ҫ�鿴��csv�����ļ�����demoprj-devc->output�ļ����£�Ȼ��ʹ��File�˵��µ�Open����ʹ�ÿ�ݼ�Ctrl-N�򿪲鿴ҳ�棬�����ʾ���ļ�����ʹ") ;
		drawLabel(MaxX-interval*68,MaxY-interval*16,"���������ܴ�output�ļ����µ������ļ���������ӻ�ҳ�档") ; 
		drawLabel(MaxX-interval*68,MaxY-interval*18,"�����ڿ��ӻ�ҳ��") ;
		drawLabel(MaxX-interval*68,MaxY-interval*20,"��ѡҳ�����Ͻǵ������������ﵽ��ʾ�����ص�ǰ���ڵ����ݵ�Ŀ�ģ���ʹ�����·�ҳ�����������������Ŷ����ѡ���½����������������ͼ������ʾ������ݵĿ��ӻ�") ;
		drawLabel(MaxX-interval*68,MaxY-interval*22,"���ܡ�����ҳ���ҷ������ʾ�ɽ���Ԥ�⹦�ܵ�ʵ��Ŷ������϶�X��Y��ʵ��ͼ������Ź��ܣ�����������ʾ��ǰ�������ݣ���ʹ�����Ͻǻָ����ܼ����ָ�ͼ���ʼ״") ;
		drawLabel(MaxX-interval*68,MaxY-interval*24,"̬��") ;
